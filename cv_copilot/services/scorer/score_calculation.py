...
def calc_skill_matches(skills: List[dict], skill_type: str) -> int:
    ...
    yes_match_weight: int = 1
    partial_match_weight: int = 0.5
    no_match_weight: int = 0
...
async def score_calculation( parsed_text: ParsedTextModel) -> dict:
    ...
    # Defining weights
    result: int = 0 
    required_skills_weight: int = 0.8
    nice_to_have_skills_weight: int = 0.2
    hard_skills_weights: int = 0.5
    soft_skills_weights: int = 0.5

    # Calc total for each skill/type
    total_required_hard_skills: int = len(required_skills['hard_skills']) if required_skills['hard_skills'] else 0
    total_required_soft_skills: int = len(required_skills['soft_skills']) if required_skills['soft_skills'] else 0
    total_nice_to_have_hard_skills: int = len(nice_to_have_skills['hard_skills']) if nice_to_have_skills['hard_skills'] else 0
    total_nice_to_have_soft_skills: int = len(nice_to_have_skills['soft_skills']) if nice_to_have_skills['soft_skills'] else 0

    # Calc score for each skill/type
    score_required_hard_skills: float = calc_skill_matches(required_skills, 'hard_skills')
    score_required_soft_skills: float = calc_skill_matches(required_skills, 'soft_skills')
    score_nice_to_have_hard_skills: float = calc_skill_matches(nice_to_have_skills, 'hard_skills')
    score_nice_to_have_soft_skills: float = calc_skill_matches(nice_to_have_skills, 'soft_skills')


    # Calc final score
    if total_required_hard_skills > 0:
        result += ((score_required_hard_skills / total_required_hard_skills) * required_skills_weight * hard_skills_weights)

    if total_required_soft_skills > 0:
        result += ((score_required_soft_skills / total_required_soft_skills) * required_skills_weight * soft_skills_weights)
    
    if total_nice_to_have_hard_skills > 0:
        result += ((score_nice_to_have_hard_skills / total_nice_to_have_hard_skills) * nice_to_have_skills_weight * hard_skills_weights)

    if total_nice_to_have_soft_skills > 0:
        result += ((score_nice_to_have_soft_skills / total_nice_to_have_soft_skills) * nice_to_have_skills_weight * soft_skills_weights)
